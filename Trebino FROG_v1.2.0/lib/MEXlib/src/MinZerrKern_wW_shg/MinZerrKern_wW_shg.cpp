...
//	printf("%g, %g, %g, %g\n", double(X(0)), double(X[1]), double(X[2]), double(X[3]));
	...
	MinZerrKern_wW_shg(Esig, Et0, dZ, X);
 ...
 */
void MinZerrKern_wW_shg(const TmexArray &Esig, const TmexArray &Ew0, const TmexArray &dZ, TmexArray &X)
{
	int Nw	= Esig.size_M();
	int NW	= Esig.size_N();
	TReal mx	= 0.0;
	...
	for(int w = 0; w < Nw; ++w)
		for(int W = 0; W < NW; ++W) {
			...
			int wp = w - (W - NW/2);
			if (wp >= 0 && wp < Nw) {
				TCmplx dZdZ		= dZ[W] * dZ[wp];
				TCmplx dZE		= dZ[W] * Ew0[wp] + dZ[wp] * Ew0[W];
				TCmplx DEsig	= Ew0[W] * Ew0[wp] - Esig(w,W);
