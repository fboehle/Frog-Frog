...
{
	if (nrhs < 3)
		mexErrMsgTxt("Too few arguments in MINZERRKERN_THG.\n");

	if (nrhs > 3)
		mexErrMsgTxt("Too many arguments in MINZERRKERN_THG.\n");

	if (nrhs < 0)
		mexErrMsgTxt("Too few outputs in MINZERRKERN_THG.\n");

	if (nlhs > 1)
		mexErrMsgTxt("Too many outputs in MINZERRKERN_THG.\n");
 ...
 */
void MinZerrKern_thg(const TmexArray &Esig, const TmexArray &Et0, const TmexArray &dZ, TmexArray &X)
{
	int Nt		= Esig.size_M();
	...
	for(int t = 0; t < Nt; ++t)
		for(int tau = 0; tau < Ntau; ++tau) {
			...
			int tp = t - (tau - Ntau/2);
			if (tp >= 0 && tp < Nt) {
				TCmplx a	= Et0[t] * Et0[tp] * Et0[tp] - Esig(t,tau);
				TCmplx b	= 2.0 * Et0[t] * Et0[tp] * dZ[tp] +  dZ[t] * Et0[tp] * Et0[tp];
				TCmplx c	= 2.0 * dZ[t] * dZ[tp] * Et0[tp] + Et0[t] * dZ[tp] * dZ[tp];
				TCmplx d	= dZ[t] * dZ[tp] * dZ[tp];
